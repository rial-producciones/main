...
<form class="simpleForm" action="index.php?ver=temasnuevo" method="post" onsubmit="javascript:return validator();">
	<label>Nombre Tema</label>
	<input type="text" class="input" name="nombreTema" id="nombreTema" value="<?=$nombreTema;?>"><br />
	<label>Artista</label>
	<input type="text" id="nombreArtista"  name="nombreArtista" value="<?=$nombreArtista;?>" class="input textbox" >
...
<h2>Buscar Tema</h2>
<form class="simpleForm" action="index.php?ver=temasnuevo" method="post">
	<label>Nombre Tema</label>
...
while ($d=mysql_fetch_object($r)) {
	echo "<tr><td>{$d->nombre}</td><td>{$d->artista}</td><td><a href=\"?ver=temasnuevo&id={$d->id}\">[Editar]</a></td><td><a href=\"?ver=temasnuevo&eliminar={$d->id}\">[Eliminar]</a></td></tr>";
}
echo "</table>"
?>