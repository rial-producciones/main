...
if ($idBtn != 1) {
	...
	$selectTonos = "<span style='font-size:10px;color:#fff;'>Seleccione para Comprar</span><select id='tonoSeleccionado' name='tonos' class='tonos' ><option value='' disabled selected>Tonalidades</option> ";
...
?>
	...
	<div class="izquierdas">
		...
		<p>
			...
			$nombreTema = mysql_real_escape_string(V("tema"));
			$nombreArtista = mysql_real_escape_string(V("artista"));
		...
		WHERE t.tipo = 2 AND t.nombre = '$nombreTema'";
			}
			...
			if (V("artista") != '' && V("tema") != '')
				...
				$q .= " AND a.nombre = '$nombreArtista' AND t.nombre = '$nombreTema'";
			...
			if (mysql_num_rows($r) > 0) {
				...
				while ($d = mysql_fetch_object($r)) {
					...
					if ($idBtn != 1) {
						...
						echo "<div class='fraces' style='color:white'>Pistas Musicales ( Canciones de Karaoke , Pistas para Cantar )</div>";
					} else {
...
<?php if ($idBtn != 1) { ?>
	<script>
		document.querySelector("#tonoSeleccionado").addEventListener("change", (e) => {
			e.preventDefault()
			if (document.querySelector("#tonoSeleccionado").value != "") {
				document.querySelector("#tonoSeleccionado").setCustomValidity("")
			}
		})
	</script>
<?php } ?>
<script>
	...
	for (let i = 0; i < form_agregar.length; i++) {
		form_agregar[i].addEventListener("submit", (e) => {
			e.preventDefault()
			const tonoSeleccionado = document.querySelector("#tonoSeleccionado")
			const data = Object.fromEntries(
				new FormData(e.target)
			);
			// console.log(data)
			console.log(tonoSeleccionado);
			if (tonoSeleccionado && tonoSeleccionado.value != "") {
				fetch("../actions/agregar_a_carrito.php", {
					...
					.then(data => {
						...
						if (sessionStorage.getItem("carrito")) {
							...
							if (!repetido(carritoTemp, data.carrito[0].id, data.carrito[0].tonos)) {

								mensaje.innerHTML = `Agregado correctamente`
								...
								mensaje.innerHTML = `Ya tienes este producto en tu carrito!`
							}

							corregirPrecioUSD(carritoTemp)
						} else {
				...
				if (tonoSeleccionado != null) {
					tonoSeleccionado.setCustomValidity("")
					tonoSeleccionado.setCustomValidity("Primero debes elegir el tono!")
					tonoSeleccionado.reportValidity()
				} else {
					fetch("../actions/agregar_a_carrito.php", {
							method: "POST",
							headers: {
	...
	function repetido(arr, idItemToSearch, tonosItemToSearch) {
		let itemInChart = false
		for (let i = 0; i < arr.length; i++) {
			if (arr[i].id == idItemToSearch && arr[i].tonos == tonosItemToSearch) {
				itemInChart = true
			}
		}
		return itemInChart
